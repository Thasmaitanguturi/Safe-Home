    ...
    while True:
        ...
        if right == "" and left == "":
            ...
            if x < 200:
                ...
                x = 300
                right, left = "", ""
                save_image(frame1, "in")
        elif left:
            if x > 500:
                print("to right")
                x = 300
                right, left = "", ""
                save_image(frame1, "out")
...
def save_image(frame, folder):
    try:
        ...
        if not os.path.exists(f"visitors/{folder}"):
            os.makedirs(f"visitors/{folder}")

        
        filename = datetime.now().strftime('%Y-%m-%d-%H-%M-%S') + ".jpg"
        cv2.imwrite(f"visitors/{folder}/{filename}", frame)
        print(f"Image saved in {folder} folder.")
